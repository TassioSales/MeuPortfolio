...
Integração com Google Gemini usando Loguru

Agora a chave da API é definida manualmente pelo usuário em tempo de execução
(sem leitura de secrets/env). Use `configure_api_key(key)` antes de chamar
qualquer função que dependa da IA.
"""
...
class GeminiService:
    ...
    def _initialize_model(self):
        """Inicializa o cliente Gemini (google-genai) com a chave já configurada."""
        if not self.api_key:
            raise AIServiceError("GEMINI_API_KEY não configurada. Defina-a manualmente na interface antes de usar a IA.")
        try:
            self.client = genai.Client(api_key=self.api_key)
            self.model_name = "gemini-2.5-flash"
            masked = ("****" + self.api_key[-4:]) if isinstance(self.api_key, str) and len(self.api_key) >= 4 else "(indisponível)"
            logger.info(f"Cliente Gemini (google-genai) inicializado | key={masked} | source=manual | model={self.model_name}")
        except Exception as e:
            logger.error(f"Erro ao inicializar cliente Gemini: {str(e)}")
            raise AIServiceError(f"Falha na inicialização do cliente: {str(e)}")

    def configure_api_key(self, key: str):
        """Configura/atualiza a chave da API e reinicializa o cliente."""
        if not key or not isinstance(key, str):
            raise AIServiceError("Chave inválida. Informe uma string não vazia.")
        self.api_key = key.strip()
        self._initialize_model()
    
    def _make_api_call(self, prompt: str, stream: bool = False, use_chat: bool = False, chat_id: str = None) -> Any:
        """Faz chamada para API Gemini (google-genai) com retry logic"""
        max_retries = 3
        base_delay = 1
        start_time = time.time()
        
        if not self.client:
            raise AIServiceError("Cliente Gemini não configurado. Defina a chave da API primeiro.")
        for attempt in range(max_retries):
            try:
                # Chamada direta (o novo cliente já gerencia sessão internamente)
                ...
                # Log de sucesso com performance
                ...
                if tokens_used:
                    ...
                    log_performance(logger, "gemini_api_call", duration, tokens=tokens_used.total_token_count)
                else:
                    log_api_call(logger, "gemini_api_call", success=True)
                    log_performance(logger, "gemini_api_call", duration)
                ...
                if attempt < max_retries - 1:
                    ...
                    log_performance(logger, "gemini_api_call", duration, success=False)
                    # Dica de causa comum
                    msg = str(e)
                    if "API key not valid" in msg or "API_KEY_INVALID" in msg:
                        logger.error("Valide sua GEMINI_API_KEY. Em ambiente local, o env var GEMINI_API_KEY tem prioridade sobre st.secrets.")
                    raise AIServiceError(f"Falha após {max_retries} tentativas: {msg}")
    
    @st.cache_data(ttl=1800)  # Cache por 30 minutos
    def search_products_with_gemini(_self, product_name: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            response = _self._make_api_call(prompt)
            ...
            try:
                ...
                log_performance(logger, "search_products", duration, products_found=len(products))
            ...
            log_performance(logger, "search_products", duration, success=False)
            raise AIServiceError(f"Falha na busca de produtos: {str(e)}")
    
    @st.cache_data(ttl=3600)  # Cache por 1 hora
    def generate_tags_for_product(_self, product_name: str) -> str:
        """
        ...
        try:
            ...
            log_performance(logger, "generate_tags", duration, tags_count=len(tags_list))
            ...
            log_performance(logger, "generate_tags", duration, success=False)
            return "produto, geral"
    
    @st.cache_data(ttl=1800)
    def generate_product_summary(_self, product_name: str) -> str:
        """
        ...
        try:
            ...
            log_performance(logger, "generate_summary", duration, summary_length=len(summary))
            ...
            log_performance(logger, "generate_summary", duration, success=False)
            return f"Resumo não disponível para {product_name}."
    
    @st.cache_data(ttl=1800)
    def summarize_reviews(_self, product_name: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                log_performance(logger, "summarize_reviews", duration, sentiment_score=review_data.get('score_sentimento', 0))
            ...
            log_performance(logger, "summarize_reviews", duration, success=False)
            return {
    ...
    def analyze_offer_quality(self, price_history: List[Dict], current_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                log_performance(logger, "analyze_offer", duration, nota=analysis.get('nota', 0))
            ...
            log_performance(logger, "analyze_offer", duration, success=False)
            return {
                "nota": 5.0,
    ...
    @st.cache_data(ttl=1800)
    def compare_products(_self, product_names: List[str], user_focus: str = "") -> str:
        """
        ...
        try:
            ...
            log_performance(logger, "compare_products", duration, products_count=len(product_names))
            ...
            log_performance(logger, "compare_products", duration, success=False)
    ...
    def answer_product_question(self, product_name: str, question: str, chat_id: str = "default") -> str:
        ...
        try:
            ...
            log_performance(logger, "answer_question", duration, question_length=len(question))
            ...
            log_performance(logger, "answer_question", duration, success=False)
            return f"Desculpe, não consegui processar sua pergunta sobre {product_name}. Tente novamente."
    
    @st.cache_data(ttl=3600)
    def suggest_alert_threshold(_self, product_name: str, budget: float = None) -> float:
        """
        ...
        try:
            ...
            try:
                ...
                log_performance(logger, "suggest_threshold", duration, threshold_value=threshold)
            ...
            log_performance(logger, "suggest_threshold", duration, success=False)
            return 199.90
