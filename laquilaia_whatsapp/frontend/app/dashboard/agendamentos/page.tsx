  ...
  if (agents.length === 0) {
    return (
      <SemAgente
        ...
        paraOAdmin="Os retornos são combinados com os contatos de um agente. Crie um para começar."
      />
    );
  }
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-fg-muted">
          Os retornos combinados com o cliente. O que passou da hora e ninguém
          fechou aparece em vermelho.
        </p>
      </header>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {agents.map((agent) => (
            <button
              key={agent.id}
              type="button"
              onClick={() => router.replace(`/dashboard/agendamentos?agent=${agent.id}`)}
      ...
      {selected && <Agendamentos key={selected.id} agentId={selected.id} />}
    </div>
  );
}
