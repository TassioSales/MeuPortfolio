  ...
  if (agents.length === 0) {
    return (
      <SemAgente
        ...
        paraOAdmin="O histórico é dos leads de um agente. Crie um para começar."
      />
    );
  }
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-fg-muted">
          O que aconteceu com cada lead, e por ordem de quem. Movimento sem nome
          foi a IA.
        </p>
      </header>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {agents.map((agent) => (
            <button
              key={agent.id}
              type="button"
              onClick={() => router.replace(`/dashboard/historico?agent=${agent.id}`)}
      ...
      {selected && <Historico key={selected.id} agentId={selected.id} />}
    </div>
  );
}
